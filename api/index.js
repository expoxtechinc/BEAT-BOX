...
var ENV = {
  ...
  aiRouterEnabled: process.env.AI_ROUTER_ENABLED ?? "true",
  aiRouterTimeoutMs: process.env.AI_ROUTER_TIMEOUT_MS ?? "18000"
};
...
// server/stripeBoundary.ts
import { TRPCError as TRPCError3 } from "@trpc/server";
function stripeCheckoutIsReady() {
...
var chatInputSchema = z2.object({
  messages: z2.array(chatMessageSchema).min(1).max(12),
  context: z2.string().max(6e3).optional()
});
var timeoutMs = () => {
  const parsed = Number(process.env.AI_ROUTER_TIMEOUT_MS ?? ENV.aiRouterTimeoutMs);
  return Number.isFinite(parsed) ? Math.min(Math.max(parsed, 3e3), 3e4) : 18e3;
};
var enabled = () => (process.env.AI_ROUTER_ENABLED ?? ENV.aiRouterEnabled) !== "false";
...
var textFromOpenAIResponse = (payload) => {
...
var providerMessages = (messages, context) => [
...
async function callOpenAICompatible(provider, messages) {
  const config = provider === "groq" ? { key: process.env.GROQ_API_KEY, model: process.env.GROQ_MODEL ?? "llama-3.3-70b-versatile", url: "https://api.groq.com/openai/v1/chat/completions" } : { key: process.env.OPENROUTER_API_KEY, model: process.env.OPENROUTER_MODEL ?? "deepseek/deepseek-chat-v3-0324:free", url: "https://openrouter.ai/api/v1/chat/completions" };
  ...
  const text2 = textFromOpenAIResponse(payload);
  if (!text2) throw new Error(`${provider} returned no text`);
  return { text: text2, provider, model: config.model };
}
async function callManus(messages) {
  const payload = await invokeLLM({ model: void 0, messages, maxTokens: 900 });
  const content = payload.choices[0]?.message?.content;
  const text2 = typeof content === "string" ? content.trim() : "";
  if (!text2) throw new Error("Built-in provider returned no text");
  return { text: text2, provider: "manus", model: payload.model || "built-in" };
}
var providers = [
...
var isProviderConfigured = (name) => name === "gemini" ? Boolean(process.env.GEMINI_API_KEY) : name === "groq" ? Boolean(process.env.GROQ_API_KEY) : name === "openrouter" ? Boolean(process.env.OPENROUTER_API_KEY) : Boolean(ENV.forgeApiKey);
var configuredProviders = () => providers.filter((provider) => isProviderConfigured(provider.name));
var maxAttemptsPerProvider = 2;
async function routeWithFallback(messages, providerList = providers, attempts = maxAttemptsPerProvider, providerTimeoutMs = timeoutMs()) {
  const failures = [];
...
var aiRouter = router({
  chat: protectedProcedure.input(chatInputSchema).mutation(async ({ input }) => {
    if (!enabled()) throw new TRPCError4({ code: "INTERNAL_SERVER_ERROR", message: "BeatBox AI is temporarily disabled." });
    ...
    try {
      ...
      throw new TRPCError4({ code: "INTERNAL_SERVER_ERROR", message: "No BeatBox AI provider is available right now." });
    }
  }),
...
async function runStoryExpiryCleanup(req, res) {
  try {
    ...
    const paths = (data || []).map((item) => item.path).filter(Boolean);
    if (paths.length) {
...
function createApp() {
  ...
  app2.post("/api/scheduled/expire-stories", runStoryExpiryCleanup);
  app2.use("/api/trpc", createExpressMiddleware({ router: appRouter, createContext }));
  return app2;
}

// server/vercel-api-entry.ts
var app = createApp();
