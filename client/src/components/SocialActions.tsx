import { Bookmark, Copy, Facebook, Globe2, Heart, LockKeyhole, MessageCircle, MoreHorizontal, Repeat2, Send, Share2, ShieldAlert, Sparkles, UserRoundX, Trash2, Users } from "lucide-react";
...
export function SocialActions({ postId, userId, liked, bookmarked, likeCount, commentCount, shareCount, onLike, onSave, onComment, onRepost, onReport, onNotInterested, onDelete, audienceLabel, onShared }: Props) {
  ...
  return <div className="community-post__action-wrap"><div className="community-post__actions" aria-label="Post actions">
    ...
    <button type="button" className="text-button" onClick={() => setOpen(value => !value)} aria-label="More post actions"><MoreHorizontal size={15} /></button>
  </div>{reactionOpen && <div className="share-menu reaction-menu" role="menu" aria-label="Choose reaction">{[["like","Like"],["love","Love"],["haha","Haha"],["wow","Wow"],["sad","Sad"],["angry","Angry"]].map(([value, label]) => <button key={value} type="button" onClick={() => void chooseReaction(value)} role="menuitem">{label}</button>)}</div>}{open && <div className="share-menu" role="menu"><button type="button" onClick={() => void share("copy")} role="menuitem"><Copy size={14} /> Copy link</button>{typeof navigator !== "undefined" && "share" in navigator && <button type="button" onClick={async () => { try { await navigator.share({ title: "BeatBox post", url }); setMessage("Share sheet opened."); } catch { /* cancelled */ } }} role="menuitem"><Send size={14} /> Device share</button>}<button type="button" onClick={() => void share("whatsapp")} role="menuitem"><Send size={14} /> WhatsApp</button><button type="button" onClick={() => void share("facebook")} role="menuitem"><Facebook size={14} /> Facebook</button><button type="button" onClick={() => void share("x")} role="menuitem"><Share2 size={14} /> X</button><button type="button" onClick={() => void share("telegram")} role="menuitem"><Send size={14} /> Telegram</button></div>}{audienceLabel && <small className="social-audience-label" title={`Post audience: ${audienceLabel}`} aria-label={`Post audience: ${audienceLabel}`}>{audienceLabel === "Friends" ? <Users size={13} aria-hidden="true" /> : audienceLabel === "Only me" ? <LockKeyhole size={13} aria-hidden="true" /> : <Globe2 size={13} aria-hidden="true" />}<span>{audienceLabel}</span></small>}{open && <div className="share-menu post-more-menu" role="menu" aria-label="More post actions">{onNotInterested && <button type="button" onClick={onNotInterested} role="menuitem"><UserRoundX size={14} /> Not interested</button>}{onDelete && <button type="button" onClick={onDelete} role="menuitem"><Trash2 size={14} /> Delete post</button>}<button type="button" onClick={onReport} role="menuitem"><ShieldAlert size={14} /> Report post</button></div>}{message && <small className="form-success" role="status">{message}</small>}</div>;
}
