...
import { Bookmark, Camera, Loader2, Music2, UserRound } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [profileResult, savedResult] = await Promise.all([
        supabase.from("profiles").select("id,username,display_name,bio,avatar_url,role,country").eq("id", user.id).maybeSingle(),
