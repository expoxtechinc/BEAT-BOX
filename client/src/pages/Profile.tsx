...
import { CreatorRoleOnboarding } from "@/components/CreatorRoleOnboarding";

type ProfileRow = { id: string; username: string | null; display_name: string | null; bio: string | null; avatar_url: string | null; cover_url: string | null; role: string | null; country: string | null; professional_mode?: boolean | null; creator_roles?: Array<"listener" | "artist" | "producer" | "creator">; primary_creator_role?: "listener" | "artist" | "producer" | "creator" | null; creator_genres?: string[]; creator_onboarding_completed?: boolean | null };
...
export default function Profile() {
  ...
  const { user, profile: authProfile, loading: authLoading } = useSupabaseAuth();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [profileResult, savedResult] = await Promise.all([
        supabase.from("profiles").select("id,username,display_name,bio,avatar_url,cover_url,role,country,professional_mode,creator_roles,primary_creator_role,creator_genres,creator_onboarding_completed").eq("id", user.id).maybeSingle(),
  ...
  const { pullDistance, refreshing, ready } = usePullToRefresh(async () => {
    if (!user) return;
    const [profileResult, savedResult] = await Promise.all([
      supabase.from("profiles").select("id,username,display_name,bio,avatar_url,cover_url,role,country,professional_mode,creator_roles,primary_creator_role,creator_genres,creator_onboarding_completed").eq("id", user.id).maybeSingle(),
      supabase.from("content_bookmarks").select("content_id,created_at,content_items!inner(id,title,content_type,price,currency,cover_path,seller_id,created_at,status)").eq("user_id", user.id).eq("content_items.status", "published").order("created_at", { ascending: false }),
    ]);
    if (profileResult.error) setError(profileResult.error.message); else setProfile(profileResult.data as ProfileRow | null);
    if (!savedResult.error) setSavedBeats((savedResult.data || []).map((row) => {
      const item = Array.isArray(row.content_items) ? row.content_items[0] : row.content_items;
      return item && item.status === "published" ? item as SavedBeat : null;
    }).filter(Boolean) as SavedBeat[]);
  ...
  const uploadCover = async (file: File) => {
    if (!user) return;
    if (!file.type.startsWith("image/")) { setError("Choose an image file for your cover."); return; }
    if (file.size > 10 * 1024 * 1024) { setError("Cover images must be 10 MB or smaller."); return; }
    if (!navigator.onLine) { setError("You are offline. Cover uploads require an internet connection."); return; }
    setCoverUploading(true); setError(null); setMessage(null);
    const path = `${user.id}/${crypto.randomUUID()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, "-")}`;
    try {
      await uploadResumable({ bucket: "profile-covers", objectPath: path, file, onProgress: () => undefined });
      const coverUrl = supabase.storage.from("profile-covers").getPublicUrl(path).data.publicUrl;
      const update = await supabase.from("profiles").update({ cover_url: coverUrl }).eq("id", user.id);
      if (update.error) throw update.error;
      setProfile(current => current ? { ...current, cover_url: coverUrl } : current);
      setMessage("Profile cover updated.");
    } catch (uploadError) {
      setError(uploadError instanceof Error ? uploadError.message : "Cover upload failed. Retry when online.");
  ...
  return <section className="dashboard-page profile-page"><div className="container">
    ...
    <CreatorRoleOnboarding profile={authProfile || profile} compact={Boolean((authProfile || profile)?.creator_onboarding_completed)} onComplete={() => setMessage("Creator identity saved. Your role-specific tools are ready.")} />
    <div className="pull-refresh" style={{ height: pullDistance ? `${Math.min(pullDistance, 72)}px` : 0 }} aria-live="polite"><span>{refreshing ? "Refreshing…" : ready ? "Release to refresh" : pullDistance ? "Pull to refresh" : ""}</span></div><div className="profile-grid">
      <article className="dashboard-panel profile-card"><div className="profile-cover" style={profile?.cover_url ? { backgroundImage: `url(${profile.cover_url})` } : undefined}><label className="profile-cover__action" htmlFor="cover-file"><Camera size={15} /> {coverUploading ? "Uploading cover…" : "Change cover"}</label><input id="cover-file" type="file" accept="image/png,image/jpeg,image/webp" hidden disabled={coverUploading} onChange={event => { const file = event.target.files?.[0]; if (file) void uploadCover(file); event.currentTarget.value = ""; }} /></div><div className="profile-avatar-wrap">{profile?.avatar_url ? <img className="profile-avatar" src={profile.avatar_url} alt={`${displayName} avatar`} /> : <div className="profile-avatar profile-avatar--fallback">{initials}</div>}<label className="avatar-upload" htmlFor="avatar-file"><Camera size={15} /> {uploading ? `Uploading ${uploadProgress}%…` : "Change avatar"}</label>{uploading && <div className="upload-progress profile-upload-progress" aria-live="polite"><div className="upload-progress__label"><span>Avatar upload</span><b>{uploadProgress}%</b></div><progress max="100" value={uploadProgress}>{uploadProgress}%</progress></div>}<input id="avatar-file" type="file" accept="image/png,image/jpeg,image/webp" hidden disabled={uploading} onChange={event => { const file = event.target.files?.[0]; if (file) void uploadAvatar(file); event.currentTarget.value = ""; }} /></div><h2>{displayName}</h2><p className="muted-copy">{profile?.bio || "Add a bio from Account settings to tell creators what moves you."}</p><div className="profile-meta"><span>{profile?.role || "listener"}</span>{profile?.professional_mode && <span><BadgeCheck size={13} /> Professional mode</span>}{profile?.country && <span>{profile.country}</span>}<span>{user.email}</span></div><button className="button button--small button--ghost" type="button" onClick={() => void toggleProfessionalMode}>{profile?.professional_mode ? "Turn off professional mode" : "Enable professional mode"}</button><Link className="button button--small" href="/account">Edit profile details</Link></article>
