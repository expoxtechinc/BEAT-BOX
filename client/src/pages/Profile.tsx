...
type ProfileRow = { id: string; username: string | null; display_name: string | null; bio: string | null; avatar_url: string | null; cover_url: string | null; role: string | null; country: string | null; professional_mode?: boolean | null };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [profileResult, savedResult] = await Promise.all([
        supabase.from("profiles").select("id,username,display_name,bio,avatar_url,cover_url,role,country,professional_mode").eq("id", user.id).maybeSingle(),
