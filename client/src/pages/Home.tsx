...
function BeatBoxHome() {
  ...
  return <>
    ...
    <section className="section section--warm"><div className="container how-grid"><div><p className="eyebrow"><span /> A clearer path to release</p><h2>Your sound deserves an honest marketplace.</h2><p className="section-copy">BeatBox separates public previews from private master files, keeps payment status visible, and gives sellers direct control over every listing.</p><Link href="/help" className="button button--outline">How BeatBox works</Link></div><div className="feature-stack"><div><div className="feature-icon"><Waves /></div><h3>Preview with confidence</h3><p>Audition watermarked clips before you commit.</p></div><div><div className="feature-icon"><ShieldCheck /></div><h3>Keep masters private</h3><p>Private audio stays protected behind short-lived download links.</p></div><div><div className="feature-icon"><UploadCloud /></div><h3>Sell without waiting</h3><p>Register as a seller and publish when your work is ready.</p></div></div></div></section>
  </>;
}
