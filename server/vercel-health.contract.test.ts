...
describe("Vercel health probe contract", () => {
  ...
  const appEntrypoint = readFileSync(resolve(process.cwd(), "api/index.ts"), "utf8");
  ...
  it("isolates health from the full application while statically bundling non-health server modules", () => {
    expect(healthEntrypoint).not.toContain("server/_core/app");
    expect(appEntrypoint).toContain('import { createApp } from "../server/_core/app"');
    expect(appEntrypoint).not.toContain('import("../server/_core/app")');
  });
